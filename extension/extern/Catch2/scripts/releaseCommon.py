...
def updateReadmeFile(version):
    ...
    downloadParser = re.compile(
        r"<a href=\"https://github.com/catchorg/Catch2/releases/download/v\d+\.\d+\.\d+/catch.hpp\">"
    )
    ...
    f = open(readmePath, "w")
    for line in lines:
        line = downloadParser.sub(
            r'<a href="https://github.com/catchorg/Catch2/releases/download/v{0}/catch.hpp">'.format(
...
def updateCmakeFile(version):
    ...
    replacementRegex = re.compile(
        b"project\\(Catch2 LANGUAGES CXX VERSION \\d+\\.\\d+\\.\\d+\\)"
    )
    replacement = "project(Catch2 LANGUAGES CXX VERSION {0})".format(
        version.getVersionString()
    ).encode("ascii")
...
def performUpdates(version):
    ...
    for rep in ("automake", "tap", "teamcity", "sonarqube"):
        sourceFile = os.path.join(
            catchPath, "include/reporters/catch_reporter_{}.hpp".format(rep)
        )
        destFile = os.path.join(
            catchPath, "single_include", "catch2", "catch_reporter_{}.hpp".format(rep)
        )
