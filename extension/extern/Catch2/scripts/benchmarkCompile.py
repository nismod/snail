...
parser = argparse.ArgumentParser(
    description="Benchmarks Catch's compile times against some synthetic tests"
)
...
parser.add_argument(
    ...
    default="all",
    choices=options,
    help="What kind of benchmark to run, default: all",
)
...
parser.add_argument(
    ...
    "--catch-header",
    default="catch.hpp",
    help="Path to catch.hpp, default: catch.hpp",
)
parser.add_argument(
    "-c", "--compiler", default="g++", help="Compiler to use, default: g++"
)

parser.add_argument(
    "-f",
    "--flags",
    help='Flags to be passed to the compiler. Pass as "," separated list',
)

# Allow creating files only, without running the whole thing
parser.add_argument(
    "-g", "--generate-files", action="store_true", help="Generate test files and quit"
)
...
if args.benchmark_kind in ("all", "main"):
    print(
        "        ... compiling main, mean: {:.2f}, median: {:.2f} s".format(
            *benchmark(compile_main)
        )
    )
if args.benchmark_kind in ("all", "files"):
    print(
        "        ... compiling test files, mean: {:.2f}, median: {:.2f} s".format(
            *benchmark(compile_files)
        )
    )
if args.benchmark_kind in ("all", "link"):
    print(
        "        ... linking everything, mean: {:.2f}, median: {:.2f} s".format(
            *benchmark(link_files)
        )
    )
