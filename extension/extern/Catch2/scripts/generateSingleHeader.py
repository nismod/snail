...
def generate(v):
    ...
    globals = {"includeImpl": True, "ifdefs": 0, "implIfDefs": -1}
    ...
    def getDirsToSearch():
        return [
            os.path.join(rootPath, s)
            for s in [
                "",
                "internal",
                "reporters",
                "internal/benchmark",
                "internal/benchmark/detail",
            ]
        ]
    ...
    def parseFile(path, filename):
        ...
        for line in f:
            ...
            if m:
                ...
                if (
                    (not guardParser.match(line) or defineParser.match(line))
                    and not commentParser1.match(line)
                    and not commentParser2.match(line)
                ):
